...
LastEditTime: 2023-10-24 16:32:38
...
from PyQt5.QtWidgets import QApplication, QMainWindow, QHBoxLayout, QVBoxLayout, QWidget, QSlider, QPushButton, QLabel
...
class HeatmapWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        real_vib_layout.addWidget(QLabel('Click to play the target vibration'), 1, Qt.AlignRight | Qt.AlignCenter)

        play_stop_button = QPushButton("Play")
        play_stop_button.clicked.connect(self.playRealVib)
        real_vib_layout.addWidget(play_stop_button, 1, Qt.AlignLeft | Qt.AlignCenter)
        ...
        self.gif_label.setMinimumSize(QtCore.QSize(200, 75))
        self.gif_label.setMaximumSize(QtCore.QSize(200, 75))
        self.gif_label.setScaledContents(True)

        layout.addWidget(self.gif_label, 1, Qt.AlignCenter | Qt.AlignCenter)
        layout.addLayout(real_vib_layout)

        optimization_title = QLabel('Generated Spectrogram')
    ...
    def playRealVib(self):
        ex = np.full((1025 - self.target_spec.shape[0], self.target_spec.shape[1]), -80)#もとの音声の周波数上限を切っているので配列の大きさを合わせるために-80dbで埋めている
        spec = np.append(self.target_spec, ex, axis=0)
